...
const targetSchema = z.object({ id: z.string().uuid(), name: z.string().min(1).max(256), notes: z.string().max(10_000).default(''), kind: z.enum(['web', 'postgresql', 'redis', 'generic']), groupId: z.string().uuid().nullable(), tagIds: z.array(z.string().uuid()), sortOrder: z.number().finite(), config: z.record(z.string(), z.union([z.string(), z.number(), z.boolean()])), vaultItemIds: z.array(z.string()), pinned: z.boolean().default(false), lastAccessAt: z.string().datetime().nullable().default(null), createdAt: z.string().datetime(), updatedAt: z.string().datetime() });
