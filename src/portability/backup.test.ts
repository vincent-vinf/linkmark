...
import { parseBackup, parseKeyStoreBackup, serializeBackup, serializeKeyStoreBackup } from './backup';
...
describe('full backup seam', () => {
  it('serializes all local records into an encrypted portable string', async () => {
    ...
    const backup = await serializeBackup({ targets: [{ id: target, name: '生产', notes: '备份备注', kind: 'web', groupId: group, tagIds: [tag], sortOrder: 0, config: { url: 'https://example.com' }, vaultItemIds: [], pinned: false, lastAccessAt: null, createdAt: '2026-01-01T00:00:00.000Z', updatedAt: '2026-01-01T00:00:00.000Z' }], groups: [{ id: group, name: '工作', sortOrder: 0 }], tags: [{ id: tag, name: '重要' }], vault: new Uint8Array([1, 2, 3]).buffer }, 'secret');
...
describe('encrypted key store backup seam', () => {
  it('contains only KDBX ciphertext in the portable payload', async () => {
    const portable = await serializeKeyStoreBackup(new Uint8Array([1, 2, 3]).buffer, 'secret');
