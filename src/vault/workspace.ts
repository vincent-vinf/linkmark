...
export type RecycledRecord = { id: string; title: string; kind: 'entry' | 'key'; deletedAt: number };
...
function addVaultItemToGroup(vault: Kdbx, group: ReturnType<typeof partition>, input: VaultItemInput): string {
  const entry = vault.createEntry(group); writeKeyFields(entry, input);
  return entry.uuid.toString();
}

function writeKeyFields(record: { fields: Map<string, string | ProtectedValue> }, input: VaultItemInput): void {
  record.fields.clear(); record.fields.set('Title', input.title);
  if (input.username) record.fields.set('UserName', input.username);
  if (input.password) record.fields.set('Password', ProtectedValue.fromString(input.password));
  if (input.notes) record.fields.set('Notes', ProtectedValue.fromString(input.notes));
  for (const [name, value] of Object.entries(input.fields ?? {})) if (name.trim() && value) record.fields.set(name.trim(), ProtectedValue.fromString(value));
}
...
export function updateKey(vault: Kdbx, id: string, input: VaultItemInput): boolean {
  const record = partition(vault, '密钥').entries.find((item) => item.uuid.toString() === id);
  if (!record) return false;
...
export function getKey(vault: Kdbx, id: string): (VaultItemInput & { id: string }) | null {
  const record = partition(vault, '密钥').entries.find((item) => item.uuid.toString() === id);
  ...
  return {
    ...
    fields: Object.fromEntries([...record.fields.entries()].filter(([name]) => !standard.has(name)).map(([name, value]) => [name, read(value)])),
  };
}
...
export function listRecycledRecords(vault: Kdbx): RecycledRecord[] {
  ...
  return recycleBin?.entries.map((entry) => ({ id: entry.uuid.toString(), title: read(entry.fields.get('Title')), kind: entry.fields.has(dataField) ? 'entry' : 'key', deletedAt: entry.lastModTime })) ?? [];
}
...
export function restoreRecycledRecord(vault: Kdbx, id: string): boolean {
  ...
  if (isEntry) {
    try {
      const target = JSON.parse(read(entry.fields.get(dataField))) as Target;
      const existingKeys = new Set(partition(vault, '密钥').entries.map((key) => key.uuid.toString()));
      const vaultItemIds = target.vaultItemIds.filter((keyId) => existingKeys.has(keyId));
      if (vaultItemIds.length !== target.vaultItemIds.length) entry.fields.set(dataField, ProtectedValue.fromString(JSON.stringify({ ...target, vaultItemIds, updatedAt: new Date().toISOString() })));
    } catch { return false; }
  }
  const targetGroup = isEntry ? partition(vault, '入口') : partition(vault, '密钥');
  vault.move(entry, targetGroup); return true;
}

export function permanentlyDeleteRecycledRecord(vault: Kdbx, id: string): boolean {
  const recycleBin = vault.meta.recycleBinUuid ? vault.getGroup(vault.meta.recycleBinUuid) : undefined;
...
export function purgeExpiredRecycledRecords(vault: Kdbx, now = Date.now(), retentionMs = 30 * 24 * 60 * 60 * 1000): number {
  const recycleBin = vault.meta.recycleBinUuid ? vault.getGroup(vault.meta.recycleBinUuid) : undefined;
  if (!recycleBin) return 0;
  const expired = recycleBin.entries.filter((entry) => entry.lastModTime <= now - retentionMs);
  for (const record of expired) recycleBin.entries.splice(recycleBin.entries.indexOf(record), 1);
...
export function deleteWorkspaceGroup(vault: Kdbx, groupId: string): boolean {
  ...
  for (const entry of getWorkspace(vault).entries.filter((item) => item.groupId === groupId)) updateEntry(vault, { ...entry, groupId: null, updatedAt: new Date().toISOString() });
  setWorkspaceMetadata(vault, { ...state, groups: state.groups.filter((group) => group.id !== groupId) });
  return true;
}

export function deleteWorkspaceTag(vault: Kdbx, tagId: string): boolean {
  const state = metadata(vault);
  if (!state.tags.some((tag) => tag.id === tagId)) return false;
  for (const entry of getWorkspace(vault).entries.filter((item) => item.tagIds.includes(tagId))) updateEntry(vault, { ...entry, tagIds: entry.tagIds.filter((id) => id !== tagId), updatedAt: new Date().toISOString() });
  setWorkspaceMetadata(vault, { ...state, tags: state.tags.filter((tag) => tag.id !== tagId) });
  return true;
}
...
export function mergeWorkspace(vault: Kdbx, incoming: Kdbx): void {
  initializeWorkspace(vault); initializeWorkspace(incoming);
  const local = getWorkspace(vault); const source = getWorkspace(incoming);
  const groupIds = new Map<string, string>(); const tagIds = new Map<string, string>(); const keyIds = new Map<string, string>();
  const groups = [...local.groups]; const tags = [...local.tags];
  for (const group of source.groups) { const id = crypto.randomUUID(); groupIds.set(group.id, id); groups.push({ ...group, id, sortOrder: groups.length }); }
  for (const tag of source.tags) { const id = crypto.randomUUID(); tagIds.set(tag.id, id); tags.push({ ...tag, id }); }
  setWorkspaceMetadata(vault, { groups, tags });
  for (const key of source.keys) { const detail = getKey(incoming, key.id); if (detail) keyIds.set(key.id, addKey(vault, detail)); }
  for (const entry of source.entries) {
    const { id: _id, createdAt: _createdAt, updatedAt: _updatedAt, ...copy } = entry;
    addEntry(vault, createTarget({ ...copy, groupId: entry.groupId ? groupIds.get(entry.groupId) ?? null : null, tagIds: entry.tagIds.flatMap((id) => tagIds.get(id) ?? []), vaultItemIds: entry.vaultItemIds.flatMap((id) => keyIds.get(id) ?? []) }));
  }
}
