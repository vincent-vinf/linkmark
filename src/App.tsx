...
export default function App() {
  ...
  const removeGroup = async (group: Group) => { if (!window.confirm(`删除分组“${group.name}”？其中入口将移入默认分组。`)) return; await db.targets.bulkPut(deleteGroup(targets, group.id)); await db.groups.delete(group.id); if (activeGroup === group.id) setActiveGroup(null); await reload(); };

  const removeTarget = async (id: string) => {
    if (!window.confirm('删除此入口？关联的秘密不会被删除。')) return;
  ...
  return <main className="shell">
    <aside className={`sidebar ${menuOpen ? 'open' : ''}`}>
      ...
      <button className="new-button" onClick={() => { setEditingTarget(null); setEditorOpen(true); }}>＋ 新建入口</button>
      <nav>
    ...
    <section className="content">
      <header>
        <div><button className="mobile-menu" onClick={() => setMenuOpen(!menuOpen)}>☰</button><p className="eyebrow">LOCAL-FIRST WORKSPACE</p><h1>{activeGroup ? groups.find((group) => group.id === activeGroup)?.name : '所有入口'}</h1></div>
      ...
      <label className="search"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={vaultUnlocked ? '搜索入口、秘密标题或账号…' : '搜索名称、连接主机或数据库…'} /></label>
      <div className="toolbar"><span>{visible.length} 个入口</span><span><label className="sort-label">排序 <select value={sortMode} onChange={(event) => setSortMode(event.target.value as typeof sortMode)}><option value="manual">手动</option><option value="pinned">置顶优先</option><option value="recent">最近访问</option><option value="name">名称</option><option value="updated">最近更新</option></select></label><button aria-pressed={viewMode === 'list'} onClick={() => setViewMode(viewMode === 'cards' ? 'list' : 'cards')}>{viewMode === 'cards' ? '列表' : '卡片'}</button><button onClick={() => { setEditingTarget(null); setEditorOpen(true); }}>新建</button></span></div>
      ...
      </article>)}</div> : <div className="empty"><span>✦</span><h2>建立你的第一个入口</h2><p>网站、PostgreSQL、Redis 与通用连接资料都会保存在这台设备上。</p><button onClick={() => { setEditingTarget(null); setEditorOpen(true); }}>新建入口</button></div>}
...
function TargetEditor({ target, groups, onClose, onSaved }: { target: Target | null; groups: Group[]; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const save = async () => {
    ...
    if (!validateTargetConfig(kind, nextConfig)) return setError('入口配置无效，凭据、DSN 和 URI 请保存到关联的 Vault Item。');
  ...
  return <div className="modal-backdrop" role="presentation"><section ref={dialogRef} className="modal" role="dialog" aria-modal="true" aria-label={target ? '编辑入口' : '新建入口'}><div className="modal-heading"><div><p className="eyebrow">{target ? 'EDIT ENTRY' : 'NEW ENTRY'}</p><h2>{target ? '编辑入口' : '添加入口'}</h2></div><button onClick={onClose}>×</button></div>
